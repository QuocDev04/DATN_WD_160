...
const ArticleAdd = () => {
    ...
    const { mutate:Article, isPending } = useMutation({
        mutationFn: async (data: any) => {
            try {
                return await instance.post("/article", data);
            } catch (error) {
        ...
        onSuccess: () => {
            messageApi.open({
                type: "success",
                content: "Bạn thêm bài viết thành công",
            });
            form.resetFields();
        },
        onError: (error) => {
            messageApi.open({
                type: "error",
                content: `Bạn thêm bài viết thất bại: ${(error as any).message || "Vui lòng thử lại sau!"}`,
            });
        },
    });
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <Form.Item name="imgArticle"
                            rules={[
                                {
                                    required: false,
                                    message: "Ảnh bài viết bắt buộc phải có",
                                },
                            ]}
                        >
