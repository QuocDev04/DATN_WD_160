...
export const getAllComment = async (req, res) => {
  try {
    const getAll = await Evaluate.find().populate("user");
    res.json(getAll);
  } catch (error) {
    console.error("Error adding comment:", error);
    res
...
export const delComment = async (req, res) => {
  try {
    const del = await Evaluate.findByIdAndDelete(req.params.id)
    res.json(del)
  } catch (error) {
...
export const putComment = async (req,res) => {
  try {
      const room = await Evaluate.findByIdAndUpdate(req.params.id, req.body)
      res.json(room)
  } catch (error) {
      res.status(StatusCodes.BAD_REQUEST).json({
          message:"fix"
